...
void main() {
	...
	vec3 sunsetFogColor = vec3(0.8, 0.56, 0.5)*1.2;
	vec3 noonFogColor = vec3(0.5, 0.76, 0.8)*1.2;
	vec3 sunsetSkyColor = vec3(0.1, 0.7, 1.0)*0.9;

	vec3 customSkyColor = (sunsetSkyColor*TimeSunrise + skyColor*TimeNoon + sunsetSkyColor*TimeSunset + nightColor*TimeMidnight);
	vec3 customFogColor = (sunsetFogColor*TimeSunrise + noonFogColor*TimeNoon + sunsetFogColor*TimeSunset + nightFogColor*TimeMidnight);
